...
const Hero = () => {
  ...
  const handleWindowResize = () => {
    if (sectionContainerRef.current && sloganContainerRef.current && imageContainerRef.current) {
      if (document.body.clientWidth >= TAILWIND_LG_WIDTH) {
        sloganContainerRef.current.parentElement?.clientWidth;
        const mx = sectionContainerRef.current.clientWidth - sloganContainerRef.current.clientWidth;
        // 160 and 60 have a better spacing lookings.
        imageContainerRef.current.style.left = `${sloganContainerRef.current?.clientWidth + mx / 2 - 160}px`;
        imageContainerRef.current.style.height = `${sectionContainerRef.current.clientHeight + 60}px`;
      } else {
  ...
  return (
    <div className="relative pt-10 pb-4 mx-auto w-full flex flex-col justify-start items-start sm:py-24" ref={sectionContainerRef}>
      <Container className="relative w-full z-10 flex justify-center">
        <div className="relative max-w-2xl lg:max-w-3xl" ref={sloganContainerRef}>
          ...
          <div className="mt-6 space-y-6 font-display text-2xl tracking-tight text-black">
            ...
            <p>您也可以只是好奇，从这里开始了解开发者，这群靠着一台电脑就能创造整个世界的灵魂。</p>
          </div>
          ...
          <dl className="mt-10 grid grid-cols-1 gap-y-6 gap-x-10 sm:mt-16 sm:gap-y-10 sm:gap-x-16 sm:text-center lg:auto-cols-auto lg:grid-flow-col lg:grid-cols-none lg:justify-start lg:text-left">
            {[["展会地点", "上海市静安区威海路 696 号 WeWork 中国旗舰店"]].map(([name, value]) => (
              <div key={name}>
                <dt className="font-mono text-base text-slate-900">{name}</dt>
                <dd className="mt-0.5 text-2xl font-semibold tracking-tight text-black">{value}</dd>
              </div>
            ))}
          </dl>
        </div>
      </Container>
      <div className="mt-8 shrink-0 h-auto w-full lg:absolute lg:left-full lg:h-full lg:-mt-24 lg:w-full" ref={imageContainerRef}>
        <img className="h-full w-auto max-w-full" src={skateboardImage.src} alt="" />
      </div>
    </div>
  );
};
